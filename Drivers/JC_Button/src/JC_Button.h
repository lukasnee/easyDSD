...
#else
	// constructor is similar to Button, but includes the initial state for the toggle.
		  ToggleButton(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin, bool initialState=false, uint32_t dbTime=25, uint8_t puEnable=true, uint8_t invert=true)
			  : Button(GPIOx, GPIO_Pin, dbTime, puEnable, invert), m_toggleState(initialState) {}
#endif
